...
- **A fractional `search_location` `limit` returned no results.** The value passed the clamp as a float, and Nominatim and Open-Meteo reject a non-integer limit with an HTTP 400, so the user saw `No locations found`. The value is now truncated before the clamp and never rejected (`2.5` → `2`, `0.5` → `1`, `50.7` → `50`), and the schema declares `limit` as an integer. Integer limits behave exactly as before. (`src/handlers/locationHandler.ts`, `src/server/weatherServer.ts`, `tests/unit/search-location-fractional-limit.test.ts`, `docs/TOOLS.md`)
