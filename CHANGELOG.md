...
- **An unreadable locations file read as "no saved locations" and was then overwritten.** Any failure to read or parse the file — a truncated write, a hand edit with a syntax error, a permissions problem, a path that is a directory — was swallowed and replaced with an empty store, which the next save persisted over the original bytes. "Empty" and "unreadable" are different answers and now render differently: `ENOENT`, and only `ENOENT`, means an empty store, and every other case throws `LocationStoreUnreadableError` from every read and every write, `clear()` included. The message names the path, says the file was not modified, and gives both remedies — repair the JSON, or move the file aside. **The file is never renamed, copied, quarantined or overwritten by the server.** Because the file is no longer cached, a hand repair takes effect on the next call with no restart. A zero-byte file is unreadable like any other: treating it as empty would let a new instance that caught an older one mid-write replace the whole file with one entry. (`src/services/locationStore.ts`, `docs/ERROR_HANDLING.md`)
- **A save that failed to reach disk was still believed by the instance that made it.** The cache was mutated before the write, so after a failed `save_location` the same instance reported the location as saved while the file on disk had never received it. With the cache gone, a failed write leaves nothing behind to believe. (`src/services/locationStore.ts`)
- **A concurrent write could truncate the file, and a reader landing in that window destroyed it.** The store wrote in place with `writeFileSync`, which truncates and then writes; a reader between the two saw zero bytes, failed the parse, and took the reset-to-empty path above. Saves now go to a uniquely named temp file in the same directory, are `fsync`ed, and are `rename`d over the target, so a reader sees the old file or the new one and never a partial. On any failure the temp file is removed and the target is untouched — there is deliberately no fallback to an in-place write. Two contracts come with the rename: **permission bits survive**, so a file you set to `0600` stays `0600`; and **a symlinked `locations.json` stays a symlink**, with its target replaced. The target is resolved by walking the link chain with `lstatSync` and `readlinkSync` rather than with `realpathSync`, which reports the same error for a path that is absent and for a symlink whose target is absent — a dotfile manager or synced folder pointing at a not-yet-created file would otherwise have lost its symlink on the first save. The JSON buffer is written to completion with every return value checked, so a short write under disk pressure or a signal cannot be `fsync`ed and `rename`d in as truncated, unreadable JSON reported as a successful save. (`src/services/locationStore.ts`)
- **`WEATHER_DEFAULT_LOCATION` set to a saved alias was geocoded as a place name when the file was unreadable.** The default-location resolver tried the value as a saved alias inside a bare `catch` and fell through to the geocoder on any failure, so an unreadable file produced weather for wherever Nominatim puts "home" rather than an error. A wrong-place forecast is worse than an error and gives the user nothing to act on. The refusal is now rethrown; every other failure still falls through, so the narrowing did not widen. (`src/utils/locationResolver.ts`)
- **A metadata edit made while a geocoded re-save was in flight was silently overwritten.** `save_location` read the existing entry, then awaited the geocoder on a full re-save by `location_query`, then restored omitted `notes`, `activities`, `description` and `alternateNames` from that pre-await snapshot — so a second client's edit during the network round trip was lost. The merge base is now re-read after the last await and immediately before the write, with no await between the two. (`src/handlers/savedLocationsHandler.ts`)
- **Every running client must be restarted after upgrading for this protection to hold — an older instance still writes from its stale cached copy and will delete saves made by a fixed one.** MCP clients spawn the server at session start, so a rebuild or an upgrade alone is invisible to an already-running session.
...
- **A report with no marine-model cell now says so, and says what to do instead.** Open-Meteo Marine answers HTTP 200 with a null wave height where its model has no cell, so `city_name: "Monterey, California"` — which geocodes to the Monterey County centroid, about 60 km inland — returned a correct but blank `Unknown` report with nothing explaining why a coastal city produced no data. The no-data branch now carries one italic note. For a geocoded place name it explains that a place name resolves to a land centroid while the marine model covers ocean and large-lake water cells only, and suggests passing `latitude`/`longitude` for a point just offshore; for a coordinate request it says the same thing without the resolution clause. It restates neither the resolved place nor its coordinates — the `**Location:**` header already prints both directly above it. It claims **only** what the server actually knows — that the upstream returned no cell, and that the point came from a place name. It does not say the point is on land, how far away the coast is, or where the nearest cell is: this repository holds no coastline geometry, and the roadmap's original phrasing ("within a few km of a coastline") would have been a claim the server cannot compute. Nor does it retry with `cell_selection=sea`, which was measured against four points during design and returned the identical cell every time. Note the wording is "ocean and large-lake water cells", not "ocean": the model was measured to cover Lake Michigan, Lake Superior and the Caspian Sea. The note is withheld from a server-default location even when one carries a place name, because `WEATHER_DEFAULT_LOCATION` reports the same `default` source for a saved alias, a bare coordinate pair and a geocoded name alike — so naming a default's provenance would invent it for a coordinate the user saved by hand. Three separate contracts pin that, one per shape a default comes in. (`src/utils/marine.ts`, `src/handlers/marineConditionsHandler.ts`, `tests/unit/marine-render-parity.test.ts`, `docs/TOOLS.md`)

### Changed

- **The sea-state description is now two sentences rather than one run-on.** `getSafetyAssessment` built its description by concatenating the rung name with a context clause that begins with a capital, so the rendered line read `Very rough Mixed wind and swell conditions.` — and, once the rungs gained the WMO code table's parentheticals, `Smooth (wavelets) Mixed wind and swell conditions.` The separator is now a full stop, applied inside the utility rather than at either render site, so the line reads `Very rough. Mixed wind and swell conditions.` and, with no context clause, `Slight.` Cosmetic and long-standing — the previous release's QA record asked that it be fixed the next time this formatter was opened. Both paths print it under a new `Sea state:` label. (`src/utils/marine.ts`, `tests/unit/marine-render-parity.test.ts`)
...
- **The miles figure mapped to two danger tiers at every seam.** Both distance lines converted the **raw** kilometres and printed miles at one decimal — coarser than the kilometre figure beside them — while the tier keys on the **displayed** kilometres. So `5.0 km (3.1 mi)` read EXTREME DANGER and `5.1 km (3.1 mi)` read HIGH ALERT: one printed miles figure spanning two tiers, at each of the three seams. One helper now derives the miles from `displayValue(km, 1)` at **two** decimals, on both the NIFC and FIRMS render sites, using `kmToMiles` from `src/utils/distance.ts` instead of an inline literal. Adjacent displayed kilometres differ by 0.1 km = 0.0621 mi, and a 0.01 mi grid cannot collapse two values 0.062 apart, so the miles figure is an injective function of the displayed kilometres and cannot span two tiers — at every seam, by construction. The kilometre figure is byte-identical, and the two `**Search Radius:**` headers are unchanged (integer source, already finer). Locked by the miles-coherence contract across both render paths. (`src/handlers/wildfireHandler.ts`, `tests/unit/wildfire-display-coherence.test.ts`, `examples/wildfire-awareness.md`, `docs/TOOLS.md`)

## [1.30.0] - 2026-09-11

### Added

- **`get_forecast` falls back to MET Norway when Open-Meteo fails transiently outside the US.** The non-US forecast branch had no catch of any kind, so an Open-Meteo outage failed every forecast outside the United States and stripped the forecast section out of every `get_weather_summary`. It is now the one render path with no fallback that has one. The branch is deliberately the **complement** of the existing NOAA one: NOAA -> Open-Meteo answers *"NOAA does not cover this point"*, a permanent rejection, while Open-Meteo -> MET Norway answers *"Open-Meteo is not answering right now"*. Getting that backwards would fall back on a bad coordinate and hide our own 400 forever while propagating the outage the feature exists to cover, so the guard keys on `isRetryableError` rather than on `instanceof ServiceUnavailableError || instanceof RateLimitError` — the pair reads as the obvious choice and misses the unclassified-network case Open-Meteo maps to a plain retryable `ApiError`. A forced `source: "openmeteo"` still propagates, because the caller named that authority, and there is **no `source: "metno"`**: the tool schema is unchanged. If MET Norway also fails, the **original Open-Meteo error** propagates and met.no's message never surfaces; the catch nests inside the critical-alert banner's own try, so a both-down failure still carries the life-threatening-alert banner exactly as today. met.no publishes no daily product, so daily values are aggregated from its `timeseries` by a pure zero-I/O module, and three properties of that series drive the design — the 1-hour/6-hour step changes partway through at a seam that **moves by location** (h+53 to h+65 when the design was written, h+56 to h+61 when the run verified it, which is why the step is read from the timestamps per entry rather than hard-coded); `next_1_hours` carries no temperature at all, so one rendered column is fed by two upstream sources across that seam; and the final entry carries no aggregation window, so an incomplete trailing day is dropped rather than rendered as whole. Every hourly entry also carries an overlapping `next_6_hours`, so matching each window to the derived step is what keeps a precipitation sum a sum rather than a six-fold overcount. It renders through its **own** formatter, never Open-Meteo's, which hard-codes an Open-Meteo footer — routing met.no through it would misattribute the source and breach CC BY at once — carrying `*Forecast data by MET Norway (CC BY 4.0)*`, a short-horizon disclosure computed from the served day count (met.no publishes about nine days against Open-Meteo's sixteen), a sentence when an hourly request is answered with the daily view, and unit conversion: met.no serves SI unconditionally while Open-Meteo is asked in display units, so this is the only formatter in the file that converts, and it converts before it rounds. `get_weather_summary` does **not** inherit this for free — it substitutes its own argument list rather than forwarding an absent parameter — so the service is threaded explicitly into its forecast section, which is the path a default install actually exercises. All four of met.no's terms-of-service obligations are met: a contact-bearing per-service User-Agent, `If-Modified-Since` conditional requests against the `Last-Modified` it publishes (it serves no `ETag`), coordinates reduced to four decimals before they reach the URL *or* the cache key, and one request per pull with no retry layer anywhere — a 403 is treated as throttled-or-unavailable and never retried, because retrying a throttle is how a throttle becomes a block. Retention and freshness are two separate clocks, so a 304 has a parse to reuse; a 304 with no cached entry is a hard error rather than an empty result. **Every request Open-Meteo answers normally is byte-for-byte unchanged**, verified by hashing rendered output from two separately built trees probed back-to-back across the NOAA, Open-Meteo daily and Open-Meteo hourly paths in both unit systems. (`src/types/metno.ts`, `src/utils/metnoParse.ts`, `src/services/metno.ts`, `src/handlers/forecastHandler.ts`, `src/handlers/weatherSummaryHandler.ts`, `src/server/weatherServer.ts`, `src/config/cache.ts`, `tests/unit/metno-parse.test.ts`, `tests/unit/metno-service.test.ts`, `tests/unit/forecast-metno-fallback.test.ts`, `tests/unit/weather-summary-metno.test.ts`, `tests/unit/critical-alert-dispatch.test.ts`, `tests/unit/critical-alert-summary.test.ts`, `docs/TOOLS.md`, `README.md`, `CLAUDE.md`)
...
- **A NOAA hourly forecast now says what kind of product it is, and points at the faster alternative.** `source: "auto"` routes on geography alone — `isInUS` decides, and granularity is not a term in that decision. That is fine for a day-ahead question and misleading at hourly resolution, where NOAA and Open-Meteo can disagree enough to change what someone does. The case that prompted this: during an active thunderstorm with lightning 10.5 km out, `get_forecast` auto-routed to NOAA and returned 56% and 32% for the next two hours, while the same tool with `source: "openmeteo"` returned 20% and 16% — and a future-radar cross-check agreed with the second. Both numbers are honest. NOAA's gridded probability is human-adjusted over the whole grid box on the forecaster's publish cadence; Open-Meteo's comes straight off the model on the model's own faster cadence. The tool was picking one silently. Every NOAA hourly response now carries one line, above the horizon disclosure and below `**Showing:**`:
      ...
      *NOAA's hourly forecast is a human-adjusted grid product: its precipitation probability is a chance over the whole grid box, republished on the forecaster's cadence rather than the model's. For a faster-cadence model view use source: "openmeteo".*

  The routing itself is unchanged — this discloses the choice, it does not re-route, add an upstream call, or add a tool parameter. It is deliberately **not** gated on `detail`, for the same reason the horizon lines beside it are not: what the upstream product *is* does not vary with verbosity. NOAA **daily** gets no such line, because the daily product is the forecaster's own on a cadence that suits a day-ahead question and nothing about it misleads. The line renders only inside the NOAA formatter, never beside the routing predicate, so it cannot appear over Open-Meteo data — the US bounding boxes overrun the border, and Toronto, Vancouver and Windsor all route to NOAA, get rejected, and fall back to Open-Meteo. Pinned on all five paths, including that fallback, by `tests/unit/forecast-noaa-source-note.test.ts`, and read live at Washington DC (present hourly, absent daily) and Toronto (absent) in both unit systems. Nothing in the tool schema changed, so the `tools/list` payload every client loads on every turn is byte-identical. (`src/handlers/forecastHandler.ts`, `docs/TOOLS.md`, `README.md`, `tests/unit/forecast-noaa-source-note.test.ts`)

### Fixed

- **The publish-time line on a US forecast had never once rendered, and now does.** The `**Updated:**` header line existed in the code from the beginning, but `formatNOAAForecast` gated it on `forecast.properties.updated`, a field the live National Weather Service forecast API does not publish and, as far as this project has ever observed, never did. What the wire publishes is `updateTime`. The properties key set is exactly `elevation, forecastGenerator, generatedAt, periods, units, updateTime, validTimes` on both the daily and the hourly product, verified live across four NWS offices. So for the project's whole history a US forecast header was four lines with no publish time at all, and a grid could be hours stale with nothing saying so — one probe found a grid six hours forty-six minutes old, and the session that prompted this work watched a NOAA grid not refresh across 35 minutes. A US forecast at either granularity now carries the time NOAA reports:

      **Updated:** Sep 8, 2026, 1:33 PM

  This is a fix to behaviour that was already broken in production rather than a newly reachable path: the guard was load-bearing and silently false the day it was written, because the type declaration claimed a required field the wire never sent and no runtime validation existed to notice. Both fields are now declared optional, per the standing convention that every field of a third-party response is optional. **NOAA daily output gains a line**, and so does the forecast section of `get_weather_summary`, which renders through the same formatter; the hourly line above is separate and the summary never requests hourly. Every Open-Meteo path is untouched, confirmed by a byte-identity sweep against the previous release across ten live probes in both unit systems, where the diff set was exactly these two lines with no deletions anywhere. Nothing in the suite could see this before: all four NOAA forecast fixtures supplied the dead field, so the swap was byte-identical in tests and changed in production. Those fixtures now carry only what the API sends, and `tests/unit/forecast-noaa-updated-line.test.ts` pins both the render and the absent-field case. (`src/types/noaa.ts`, `src/handlers/forecastHandler.ts`, `docs/TOOLS.md`, `README.md`, `examples/`, `tests/unit/forecast-noaa-updated-line.test.ts`, `tests/integration/almanac.test.ts`)
...
- **The seventeen tool names are now declared once, so the `ENABLED_TOOLS` type guard can no longer omit a tool the type knows.** They previously appeared twice in `src/config/tools.ts` — as the `ToolName` union and again, 240 lines below, as a `validTools` array inside `isToolName`. Because that array was typed `ToolName[]`, the drift was asymmetric: adding a bogus name was a compile error, but *omitting* a real one was not, and an omitted name made `isToolName` return `false`, `resolveToolName` return `undefined`, and `ENABLED_TOOLS=that_tool` silently fail to enable a tool that exists, is registered, and dispatches correctly. Nothing failed and nothing was logged. The names are now a single frozen `TOOL_NAMES` tuple with `ToolName` derived from it, and `isToolName` tests membership of that tuple — the two surfaces are unable to disagree rather than merely tested for agreement. The compile-time direction was **demonstrated, not assumed**: deleting one name from the tuple now produces seven `TS2322` errors, at `TOOL_PRESETS.full`, `TOOL_PRESETS.all` and five `TOOL_ALIASES` entries, where the same omission previously compiled clean. A new unit test pins the two surfaces that cannot be collapsed — the `TOOL_DEFINITIONS` registry and the `CallToolRequest` dispatch — to that same list in both directions, checks that each registry entry's inner `name` field matches its key, and confirms every dispatch arm records analytics under its own name, which had been verified by hand once and never by machine. **The dispatch check is textual by design**: a `switch` has no runtime representation, so no test can enumerate its arms by importing anything; the check reads `src/index.ts` as text and **will fail if someone reformats a `case` label**. That trade-off is accepted deliberately and stated in the test file's header — a reformatted label is worth a failing test rather than a silent pass. Six mutations were shown red and restored to prove the guard is real, three of which compile cleanly and so are catchable by nothing else: a reformatted label, an arm for a tool that does not exist, and a registry entry whose advertised name disagrees with its key. **Nothing user-visible changes** — no tool, parameter, preset or alias moves, and `ENABLED_TOOLS` accepts exactly the names it accepted before. The new test is also the first in the repo to import `src/index.ts`, which builds the analytics singleton at module load; because that reads-or-writes an installation salt regardless of `ANALYTICS_ENABLED`, the test wrote `~/.weather-mcp/analytics-salt` on any machine without a local `.env` — it now hoists a fixed `ANALYTICS_SALT` and touches no home directory. (`src/config/tools.ts`, `src/index.ts`, `tests/unit/tool-name-parity.test.ts`)
