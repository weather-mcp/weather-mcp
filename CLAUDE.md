...
- **Version:** 1.30.1 (Production Ready)
- **License:** MIT
...
4. Register tool in `src/server/weatherServer.ts` (`TOOL_DEFINITIONS` + dispatch) and in `src/config/tools.ts` (`TOOL_NAMES`, presets).
   ...
   a reformatted label fails it (deliberately — a `switch` has no runtime representation to enumerate)
...
- **Version:** 1.30.1 — Production Ready ✅
- **Test Coverage:** 3,464 tests, 100% pass rate
...
- **New in v1.29.3:** Every doc count site the release writer rewrites is now validated from one shared table, so a reworded or unguarded site fails the check instead of escaping it
...
**Last Updated:** 2026-09-17 (v1.30.1)
