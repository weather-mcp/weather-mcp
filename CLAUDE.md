...
1. **get_forecast** - 7-day forecasts (NOAA/Open-Meteo, auto-select by location); `detail` output control; `compare_models: true` returns a five-model agreement comparison instead of a single forecast (v1.21.0)
...
- **Test Coverage:** 2,077 tests, 100% pass rate
