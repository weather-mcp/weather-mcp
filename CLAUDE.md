...
- **Version:** 1.20.0 (Production Ready)
- **License:** MIT
...
│   ├── ensembleSpread.ts   # Single-model member spread: percentiles, wet fractions, confidence — pure, imports only modelComparison (v1.22.0)
...
1. **get_forecast** - 7-day forecasts (NOAA/Open-Meteo, auto-select by location); `detail` output control; `compare_models: true` returns a five-model agreement comparison instead of a single forecast (v1.21.0); `ensemble_spread: true` returns one model's own member spread — ECMWF ENS, 50 members — instead of a single forecast (v1.22.0). The two flags are mutually exclusive
...
**Last Updated:** 2026-08-16 (v1.21.0 global normals hardening, unreleased)
