...
3. [get_alerts](#3-get_alerts) — Watches/warnings/advisories: US, Canada, Europe, India, Philippines, Indonesia, Japan (+ ~45 more territories with an optional key)
...
- `source` (optional): "auto" (default), "noaa" (US only), or "openmeteo" (global). **At `granularity: "hourly"` the choice is worth knowing about**, and `auto` now says which product answered. NOAA's hourly grid is a *human-adjusted* probability over the whole grid box, republished on the forecaster's cadence rather than a model run's — the `**Updated:**` line in the response header is that cadence's last tick. `source: "openmeteo"` gives a raw high-resolution model view on a faster cadence, which is also what drives the radar loops people cross-check a forecast against. The two can legitimately disagree at hourly resolution and neither is wrong, so the tool discloses the choice rather than making it silently. Daily forecasts are unaffected: the NOAA daily product is the forecaster's own on a cadence that suits a day-ahead question
...
| Japan | Japan Meteorological Agency (disaster-prevention XML feed) | Warning name in Japanese as published with an English gloss beside it where known, JMA tier (Emergency Warning / Warning / Advisory), the qualifying condition where given, the issuing office and the bulletin time. Matched to the requested point by class10 warning area; a lifted (`解除`) warning and JMA's explicit quiet marker (`発表警報・注意報はなし`) are not shown as active |
