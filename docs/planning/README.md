...
| Single-model ensemble spread (Open-Meteo ensemble API) | 📝 | Design doc [`docs/ensemble-spread-plan.md`](../ensemble-spread-plan.md) (2026-08-16), targeting v1.22.0 as `ensemble_spread: true` on `get_forecast` — ECMWF ENS, 50 members, fixed model. Originally descoped from the v1.21.0 comparison — a different product (member spread *within* one model, vs. agreement *across* models). Upstream live-verified 2026-08-16: ensemble endpoint supports daily aggregation; `precipitation_probability_max` is all-null there (compute wet-member fraction instead) |
