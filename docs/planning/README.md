...
| Global normals hardening (full-year table pull, 429 posture, predicate/render unification) | 📝 | [`docs/global-normals-hardening-plan.md`](../global-normals-hardening-plan.md) — hardens the global path everyone is on: one archive pull per location instead of per date (+2.5 % bytes, live-verified 2026-08-17), in-flight dedupe + bounded 429 retry, single `isInUS` predicate for the NCEI gate, shared render helper, float-precision storage, Feb 29 definition, and a real (non-either-way) test lock on international normals |
