...
| What else the Google Maps Platform key could unlock | 📋 | Research register, catalogue only — no feature designed: [GOOGLE_KEY_OPPORTUNITIES.md](./GOOGLE_KEY_OPPORTUNITIES.md). Follow-on to the v1.22.0 pollen fallback, which introduced the project's first GMP key. Its **#1 candidate, a global weather-alerts fallback** (Google Weather API), has since been designed and is in implementation for v1.23.0 — see the row in *International & global coverage* above. Anything promoted from here inherits the standing key policy above, and per the pollen plan's D10 each feature gets its **own** env var rather than sharing one key |
