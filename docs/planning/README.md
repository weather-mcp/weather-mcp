...
| International alerts (MeteoAlarm, MSC GeoMet, WMO CAP) | ✅ | Shipped on `feat/international-alerts` targeting v1.19.0; [`docs/plans/international-alerts-plan.md`](../plans/international-alerts-plan.md); ICR Phase 3; rest-of-world stayed out keyless (WMO feeds not production-usable) — now filled by the keyed Google fallback row below |
| Global alerts fallback via Google Weather API (optional `GOOGLE_WEATHER_API_KEY`) | ✅ | Shipped on `feat/global-alerts-fallback` targeting v1.23.0; [`docs/plans/global-alerts-fallback-plan.md`](../plans/global-alerts-fallback-plan.md). Promoted from [GOOGLE_KEY_OPPORTUNITIES](./GOOGLE_KEY_OPPORTUNITIES.md) #1, its strongest candidate. Keyed fallback on the **elsewhere branch only** — the US, Canada, and the 38 MeteoAlarm countries never contact Google, and keyless output is byte-identical. Unlike the pollen fallback this data is **contract, not garnish**: failures surface loudly rather than degrading to a possibly-false all-clear. The free-tier gate passed at live verification (one Essentials SKU, 10,000 free events/month). Live verification corrected **six** documented shapes — most consequentially the response array field, which is `weatherAlerts` not `alerts`, so the documented name returned empty for every location on Earth |
...
| ~~Global climate normals (Open-Meteo archive outside US)~~ | ✅ | **Stale idea — already shipped in v1.2.0** (discovered 2026-08-17, the FE §1.1 "no moon API" pattern): `include_normals` falls back to Open-Meteo computed 1991–2020 normals for *any* coordinates whenever NCEI is unavailable, and the project ships keyless, so the global path is what virtually every caller already uses (`src/utils/normals.ts`). Superseded by the hardening row below |
| Global normals hardening (full-year table pull, 429 posture, predicate/render unification) | ✅ | Shipped on `feat/global-normals-hardening`; [`docs/plans/global-normals-hardening-plan.md`](../plans/global-normals-hardening-plan.md) — hardened the global path everyone is on: one archive pull per location instead of per date (+2.5 % bytes, live-verified), null-sample averaging fix, in-flight dedupe + bounded 429 retry, single `isInUS` predicate for the NCEI gate (AK/HI/PR now reach NCEI), shared render helper with an aligned failure heading, float-precision storage, Feb 29 carve-out, and a real (non-either-way) test lock on international normals |
...
| Multi-model forecast comparison (Open-Meteo `models` param) | ✅ | Shipped in v1.21.0 as `compare_models: true` on `get_forecast` — not a new tool; [`docs/plans/multi-model-comparison-plan.md`](../plans/multi-model-comparison-plan.md); [FORK_DERIVED_IDEAS](./FORK_DERIVED_IDEAS.md) #4 — *rejected in 2025 as "too complex" via NOMADS; reconsidered 2026-08 via Open-Meteo, which returns all five models in one call*. `best_match` is a reference line only, excluded from every spread statistic |
...
| Single-model ensemble spread (Open-Meteo ensemble API) | ✅ | Shipped in v1.21.0 (developed as the internal v1.22.0 milestone) as `ensemble_spread: true` on `get_forecast` — not a new tool; [`docs/plans/ensemble-spread-plan.md`](../plans/ensemble-spread-plan.md). ECMWF ENS 0.25°, 50 perturbed members + control, one keyless call. A different product from `compare_models` — member spread *within* one model, vs. agreement *across* models — and the two flags are mutually exclusive. The control run is a reference line only, excluded from every statistic; `precipitation_probability_max` is all-null on the ensemble endpoint, so the wet-member fraction is the probability product |
| Caller-selectable ensemble model (`gfs_seamless` 33-day horizon, `icon_seamless`, `gem_global`, …) | 💡 | Descoped from the v1.21.0 ensemble spread: one fixed curated model per the intelligent-defaults discipline, though all the alternatives were verified working upstream. `gfs_seamless` would notably extend the horizon to ~33 days. Revisit alongside the `compare_models` model-list override above |
...
| What else the Google Maps Platform key could unlock | 📋 | Research register, catalogue only — no feature designed: [GOOGLE_KEY_OPPORTUNITIES.md](./GOOGLE_KEY_OPPORTUNITIES.md). Follow-on to the v1.22.0 pollen fallback, which introduced the project's first GMP key. Its **#1 candidate, a global weather-alerts fallback** (Google Weather API), has since shipped in v1.23.0 — see the row in *International & global coverage* above. Anything promoted from here inherits the standing key policy above, and per the pollen plan's D10 each feature gets its **own** env var rather than sharing one key |
...
| Composited imagery via MCP image content blocks (radar over base map, returned as a finished picture) | ✅ | Shipped in v1.19.0 as `composite: true` on `get_weather_imagery`; [`docs/plans/composited-imagery-plan.md`](../plans/composited-imagery-plan.md). D2 settled on NASA GIBS (OSM and Carto ruled out on terms); client survey done — image blocks are protocol-standard, inline rendering varies |
...
| Qualify the WBGT band by sun/cloud/time of day | 💡 | The ABM simplified WBGT assumes *moderately high radiation with light wind*, so the most alarming band fires most reliably at the hours the model is least applicable — a 22:45 local New Orleans probe rendered `Heat stress (Extreme)` during the v1.24.0-line sweep. The mandatory inline caveat is the disclosed mitigation and is honest, but both render paths already hold inputs that could bound it (`cloud_cover`, wind, the observation timestamp and station timezone). Deliberately not changed days before a tag; revisit as a design question, since softening a safety band is not a mechanical edit ([`docs/plans/heat-cold-stress-plan.md`](../plans/heat-cold-stress-plan.md) pre-tag review) |
...
| Release-review hardening: null-input Fosberg fabrication, US-territory wildfire routing, forced-NIFC false all-clear, METAR note advice, fire-variable blast radius, antimeridian bbox | ✅ | Rides v1.20.0 (implemented 2026-08-14, pre-tag); [`docs/plans/release-review-hardening-plan.md`](../plans/release-review-hardening-plan.md) — six findings (F1–F6) from the pre-release code review of `main...feat/global-fire-weather`, all fixed. The security review of the same diff returned no HIGH/MEDIUM findings. F7 (clustering without a spatial index) deferred — bounded by the existing 5,000-row cap |
