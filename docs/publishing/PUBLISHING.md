...
2. **Prepare the release (automated)** → Run `./scripts/update-docs-for-release.sh <patch|minor|major|X.Y.Z> ["summary"]`. This bumps `package.json` + `server.json`, promotes the CHANGELOG `[Unreleased]` section (or seeds a draft from conventional commits since the last tag), updates version, test-count (including the README badge), and tool-count references in CLAUDE.md, README.md, docs/README.md, docs/TOOLS.md, package.json, server.json, and SECURITY.md, re-renders `.github/social-preview.png` if the tool count changed (re-upload it manually at GitHub → Settings → Social preview), and runs `check-doc-versions.sh`. Tip: write CHANGELOG notes under `[Unreleased]` as you develop — the script promotes them verbatim.
...
5. **GitHub repository** - Public repo at github.com/weather-mcp/weather-mcp
...
1. Go to https://github.com/weather-mcp/weather-mcp/releases
...
See [CLIENT_SETUP.md](https://github.com/weather-mcp/weather-mcp/blob/main/docs/CLIENT_SETUP.md) for all supported clients.
...
- [README.md](https://github.com/weather-mcp/weather-mcp#readme)
- [CHANGELOG.md](https://github.com/weather-mcp/weather-mcp/blob/main/docs/releases/CHANGELOG.md)
...
MIT License - see [LICENSE](https://github.com/weather-mcp/weather-mcp/blob/main/LICENSE)
````
